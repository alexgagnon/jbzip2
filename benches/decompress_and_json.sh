...
# $'bunzip2 --keep -c ../samples/data/{i}.json.bz2 | jq -r --stream \'fromstream(1|truncate_stream(inputs)) | {filter}\'' \
# $'lbunzip2 --keep -c ../samples/data/{i}.json.lbz2 | jq -r --stream \'fromstream(1|truncate_stream(inputs)) | {filter}\'' \
